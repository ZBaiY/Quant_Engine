...
class LinearSlippage(SlippageModel):
    ...
    def apply(self, order, market_data):
        log_debug(self._logger, "LinearSlippage received order", side=order.side, qty=order.qty, impact=self.impact)
        mid = (market_data["bid"] + market_data["ask"]) / 2
        slip = self.impact * order.qty
        log_debug(self._logger, "LinearSlippage computed slippage", mid=mid, slip=slip, adjusted_price=(mid + slip if order.side == "BUY" else mid - slip))
